# Техническое задание: Накопительная система лояльности «Гофермарт»
...
| Параметр | Переменная окружения | Флаг |
|----------|---------------------|------|
| Адрес и порт запуска сервиса | `RUN_ADDRESS` | `-a` |
| Адрес подключения к базе данных | `DATABASE_URI` | `-d` |
| Адрес системы расчёта начислений | `ACCRUAL_SYSTEM_ADDRESS` | `-r` |
